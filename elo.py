...
def setup(teams, rd):
    return pd.merge(rd, teams, left_on="Aff", right_on="Team"), pd.merge(
        rd, teams, left_on="Neg", right_on="Team"
    )
...
def compute_new_elo(aff, neg, weight):
    ...
    return (
        aff[["Aff", "New_ELO", "New_Aff_ELO"]],
        neg[["Neg", "New_ELO", "New_Neg_ELO"]],
    )


def elo_merge(teams, aff, neg):
    teams = pd.merge(teams, aff, left_on="Team", right_on="Aff", how="left")
    for index, row in teams.iterrows():
        if not pd.isna(row["New_Aff_ELO"]):
            teams.at[index, "ELO"] = row["New_ELO"]
            teams.at[index, "Aff_Rounds"] += 1
            teams.at[index, "Aff_ELO"] = row["New_Aff_ELO"]
    teams = teams[["Team", "ELO", "Aff_ELO", "Neg_ELO", "Aff_Rounds", "Neg_Rounds"]]
    teams = pd.merge(teams, neg, left_on="Team", right_on="Neg", how="left")
    for index, row in teams.iterrows():
        if not pd.isna(row["New_Neg_ELO"]):
            teams.at[index, "ELO"] = row["New_ELO"]
            teams.at[index, "Neg_Rounds"] += 1
            teams.at[index, "Neg_ELO"] = row["New_Neg_ELO"]
...
decay_map = {
    **{key: math.exp(-0.1 * 3) for key in range(0, 53)},
    **{key: math.exp(-0.1 * 2) for key in range(53, 181)},
    **{key: math.exp(-0.1 * 1) for key in range(181, 245)},
    **{key: math.exp(-0.1 * 0) for key in range(245, 333)},
}

real_map = {**{key: decay_map.get(key) * weight_map.get(key) for key in range(0, 333)}}

teams = pd.read_csv("teams_ip.csv")
for i in range(100):
    for k in range(len(rds)):
        teams = elo_all(teams, results[k], real_map.get(k))
teams[["ELO", "Aff_ELO", "Neg_ELO"]] = round(teams[["ELO", "Aff_ELO", "Neg_ELO"]], 3)
teams[["Aff_Rounds", "Neg_Rounds"]] /= 100
teams.to_csv("teams_mba.csv")
